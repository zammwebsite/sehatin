...
const predictions = await model.estimateFaces(videoElement, false);
  ...
  const handleStartCheck = () => {
    if (!streamRef.current) {
        setError('Kamera belum siap. Mohon tunggu atau berikan izin.');
        return;
    }
    setStatus('checking');
    setResults(null);
    setTimeout(() => {
      ...
      const mockFatigue: 'Low' | 'Medium' | 'High' = ['Low', 'Medium', 'High'][Math.floor(Math.random() * 3)] as 'Low' | 'Medium' | 'High';
      setResults({ heartRate: mockHeartRate, fatigueLevel: mockFatigue });
      setStatus('complete');
    }, 5000); // Simulate 5-second check
  };
      ...
      case 'complete':
        return (
          <div className="absolute inset-0 bg-black bg-opacity-75 flex flex-col items-center justify-center text-white z-10 p-4 text-center">
            ...
            <p className="text-xs mt-4 text-gray-300">*Hasil ini adalah estimasi dan bukan diagnosis medis.</p>
          </div>
        );
      default:
        return null;
