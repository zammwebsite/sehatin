...
// 🔹 Lazy load model hanya di browser
useEffect(() => {
...
console.log('✅ Model BlazeFace berhasil dimuat');
} catch (err) {
...
return ( <div className="bg-white rounded-xl shadow-md p-6"> <h2 className="text-xl font-bold text-gray-800 mb-4">Cek Kesehatan via Kamera</h2> <div className="w-full aspect-video bg-gray-900 rounded-lg overflow-hidden relative flex items-center justify-center"> <video ref={videoRef} autoPlay playsInline muted className="w-full h-full object-cover" />
{renderContent()}
{!streamRef.current && !error && ( <div className="absolute inset-0 flex flex-col items-center justify-center text-white p-4 text-center"> <CameraIcon className="w-16 h-16 mb-4" /> <p>Menyalakan kamera...</p> </div>
)} </div> <div className="mt-6 flex flex-col sm:flex-row justify-center items-center sm:space-x-4 space-y-4 sm:space-y-0">
<button
...
>
{status === 'checking' ? 'Memeriksa...' : 'Mulai Pemeriksaan'} </button>
